...
kexperimento <- NA
path_train <- here("./data/transformed/paquete_premium_202011_02_ext.csv")
path_test <- here("./data/transformed/paquete_premium_202101_02_ext.csv")
path_logs <- here("./logs/experimento_1")

script_name <- "02_bayesian_optimization_ranger_1.R"
...
ksemilla_azar  <- 12345678  #Aqui poner la propia semilla
...
ranger_CrossValidation  <- function( data, param, pcampos_buenos, qfolds=5, pagrupa, semilla )
{
  divi  <- rep( 1, qfolds )
...
kbayesiana  <- paste0("../resultados_experimentos/",  kexperimento, "_rpart.RDATA" )
klog        <- paste0("../logs/",  kexperimento, "_rpart_log.txt" )
kkaggle     <- paste0("../kaggle/",kexperimento, "_", script_name , "_" )
...
dataset  <- fread(path_train, stringsAsFactors= TRUE)   #donde entreno
...
dapply   <- fread(path_test, stringsAsFactors= TRUE)   #donde aplico el modelo
dapply[ , clase_ternaria := NULL ]  #Elimino esta columna que esta toda en NA
dapply  <- na.roughfix( dapply ) # Impute Missing Values by median/mode.
